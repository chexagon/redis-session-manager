 ...
 * </ol>
 * If application logic modifies an existing session attribute, e.g., <code>Foo.class.cast(session.getAttribute("Foo")).setBar("Bar");</code>
 * the session <em>will not</em> be marked dirty. Setting {@link RedisSessionManager#setForceSaveAfterRequest(boolean)} to <code>true</code>
 * will alleviate this but can cause unnecessary persists. The preferred workaround for this (other than preventing mutation of session attributes)
 * is to enable {@link RedisSessionManager#isDirtyOnMutation()} and invoke <code>setAttribute()</code> with the existing key and new value.
 * Alternatively one could use <code>session.removeAttribute(null);</code> but this is a bit of a hack.<br>
